...
function finishShiftUpHorizontalLayersAfterEverythingLoaded() {
    1 == canFinishShiftUpHorizontalLayersAfterEverythingLoaded && (canFinishShiftUpHorizontalLayersAfterEverythingLoaded = !1,
        isPreloadShiftUpAnimationFinish = !0,
        makePageScrollable(),
...
function detectPageVerticalPosition() {
    previousPageVerticalPosition = pageVerticalPosition,
        ...
        0 > pageVerticalPosition && (pageVerticalPosition = 0),
    ...
    0 >= pageVerticalPosition && (resetVariables(),
        resetFunctions())
}
...
function moveUpLayerHorizontal() {
    if ("horizontal" == layersMovement) {
        for (var e = 0; e < layerHorizontalArray.length; e++) {
            ...
            -shiftUpLayerHorizontalDistance >= t ? (t = -shiftUpLayerHorizontalDistance,
...
function positionBalloonAndRobbyContainerHorizontally() {
    ...
    o >= a && (o = a),
        "vertical" == layersMovement ? (balloonDiv.style.left = o + "px",
            robbyContainerDiv.style.left = n + "px") : "not moving 1" == layersMovement || "not moving 2" == layersMovement ? (robbyContainerDiv.style.left = n + pageVerticalPosition - (pageDiv.offsetHeight - containerDiv.offsetHeight - distanceBetweenRobbyAndBalloon) + "px",
...
function orientRobby() {
    deltaPageVerticalPosition > 0 && (robbyFramesDiv.style.top = "0px",
        robbyEyesCloseDiv.style.left = "90px"),
    0 > deltaPageVerticalPosition && (robbyFramesDiv.style.top = "-200px",
...
function positionPlants() {
    for (var e = 0; e < plantArray.length; e++)
        1 == canAnimatePlantInformation ? plantArray[e].style.top = "100%" : plantArray[e].style.top = plantTargetTopObjectArray[e].offsetTop + "px"
}
...
function shiftRobbyFrame() {
    ...
    robbyStartFrame + counter + switcher > robbyStopFrame && (switcher = -1 * switcher),
    robbyStartFrame + counter + switcher == robbyStartFrame && (pageVerticalPositionWhenAnimateRobby1 = pageVerticalPosition),
    robbyStartFrame > robbyStartFrame + counter + switcher) {
        if (pageVerticalPositionWhenAnimateRobby2 = pageVerticalPosition,
        pageVerticalPositionWhenAnimateRobby1 == pageVerticalPositionWhenAnimateRobby2)
            return clearShiftRobbyFrameTimer(),
...
function animateInformationAndEnemiesElements() {
    if ("horizontal" == layersMovement) {
        ...
        if (1 == isRobbySwimming)
            for (var e = 0; e < seaInformationContainerArray.length; e++)
...
function changeRobotHands() {
    if (changeRobotHandsCounter >= robotHandChildrenLength)
        ...
        for (var e = 0; robotHandChildrenLength > e; e++)
...
function openAndCloseSquidHands() {
    openAndCloseSquidHandsCounter >= 8 ? (openAndCloseSquidHandsCounter = 0,
        clearInterval(moveSquidHandsTimer),
        openSquidHands(),
...
function openSquidHands() {
    ...
    for (var e = 0; e < squidHandCloseArray.length; e++)
...
function rotateAlienHands() {
    alienSteerPreviousAngle = alienSteerAngle,
        ...
        alienSteerAngle > alienSteerPreviousAngle ? alienSteerAngle > alienSteerAngleLimit && (alienSteerAngleIncrement = -1 * alienSteerAngleIncrement,
            alienSteerAngleLimit = -1 * alienSteerAngleLimit) : alienSteerAngleLimit > alienSteerAngle && (alienSteerAngleIncrement = -1 * alienSteerAngleIncrement,
...
function setSocialContainerOpacity(e) {
    e > 1 && (e = 1),
    0 > e && (e = 0);
    for (var t = $(socialContainerDiv).children().length, i = 0; t > i; i++)
        $(socialContainerDiv.children[i]).fadeTo(0, e);
    for (var n = $(socialContainerDiv.children[1]).children().length, i = 0; n > i; i++)
...
function positionSeaFloorObjectsVertically() {
    ...
    for (var e = 0; e < seaFloorBackObjectArray.length; e++)
...
function positionContactConfirmationContainer() {
    for (var e = 0; e < contactConfirmationContainerArray.length; e++)
        "not moving 1" == layersMovement || "not moving 2" == layersMovement ? contactConfirmationContainerArray[e].style.left = robbyContainerDiv.offsetLeft + "px" : contactConfirmationContainerArray[e].style.left = robbyMaxHorizontalDistance + "px",
...
function hideContactConfirmationContainer() {
    if (1 == isContactConfirmationContainerVisible) {
        for (var e = 0; e < contactConfirmationContainerArray.length; e++)
            for (var t = $(contactConfirmationContainerArray[e]).children().children().length, i = 0; t > i; i++)
...
var contentDiv = document.getElementById("content"),
    ...
    layerHorizontalArray = new Array, layerVerticalArray = new Array,
...
var canAnimatePlantInformation,
    ...
    buildingTargetLeft1 = 0, buildingTargetLeft2 = 305,
    buildingTargetLeft3 = 710, buildingEarlyPosition1 = 795,
    buildingEarlyPosition2 = 1100, buildingEarlyPosition3 = 1505,
...
var buildingLegArray = new Array, canAnimateBuildingInformation,
    buildingCounter = 0, buildingLegsTimer,
    buildingBlinkTimer, buildingEnemyFaceEyeArray = new Array,
    ...
    robotDiv = document.getElementById("robot"), animateRobotHandsTimer,
    spinRobotHandsTimer, changeRobotHandsCounter = 0,
    ...
    squidHandCloseArray = new Array, squidHandOpenArray = new Array,
    animateSquidHandsTimer, moveSquidHandsTimer,
    openAndCloseSquidHandsCounter = 0,
    alienDiv = document.getElementById("alien"), animateAlienHandsTimer,
    alienSteerDiv = document.getElementById("alien-steer"), alienSteerAngle = 0,
    alienSteerPreviousAngle,
    alienSteerAngleLimit = 15, alienSteerAngleIncrement = 5,
    canAnimateBossInformation, canAnimateRobotInformation,
    canAnimateSquidInformation, canAnimateAlienInformation,
    ...
    bubbleDiv = document.getElementById("bubble"), bubbleTimer,
    shiftUpLayerHorizontalDistance,
    shiftUpLayerHorizontalTimer, shiftDownLayerHorizontalTimer,
    ...
    fishArray = new Array, fishEyeArray = new Array,
    canAnimateFishInformation, isFishStillAnimating = !1, fishAnimateNumber = 0,
...
var skill2ContainerDiv = document.getElementById("skill-2-container"),
    crabArray = new Array, crabEyeArray = new Array,
    canAnimateCrabInformation, isCrabStillAnimating = !1, crabAnimateNumber = 0,
...
var skill3ContainerDiv = document.getElementById("skill-3-container"),
    ...
    turtleEyeArray = new Array, canAnimateTurtleInformation,
...
var pageVerticalPosition = 0, pageVerticalPositionOnTouch = 0,
    previousPageVerticalPosition = 0,
    deltaPageVerticalPosition = 0, isRobbySwimming = !1, isRobbyJumping,
    isRobbyFalling, isRobbyBelowSeaLevel = !1,
    swimUpHeight, layersMovement, elevationArray = new Array,
    elevationNumberBelowRobby = null, robbyRightEdge,
    robbyLeftEdge, distanceBetweenRobbyAndBalloon = 150,
    robbyMaxHorizontalDistance, counter = 0, switcher = 1,
    robbyStaticFrame = 0, robbyStartRunFrame = 1, robbyStopRunFrame = 2,
    robbyStartSwimFrame = 3,
    robbyStopSwimFrame = 4, robbySwimDownFrame = 5, robbyStartJumpFrame = 6,
    robbyStopJumpFrame = 7,
    robbyOneFrameWidth = 200, shiftRobbyFrameTimeInterval = 200,
    canAnimateRobbyRunSwim, robbyStartFrame,
    robbyStopFrame, shiftRobbyFrameTimer, pageVerticalPositionWhenAnimateRobby1,
    ...
    canAnimateNbaInformation, nbaBoardsCounter, nbaBoardsAnimationTimer,
    ...
    canAnimateSocialContainer, happyRobbyTimer,
    isRobbyHappy = !1,
    ...
    canHideScrollOrSwipeTextContainer = !0, scrollOrSwipeTextContainerTimer,
    ...
    waterfallTimer, touchStartX = 0, touchCurrentX = 0, touchEndX = 0,
    fireworksContainerDiv = document.getElementById("fireworks-container"),
    fireworkArray = new Array,
    fireworkSvgArray = new Array, cArray = new Array, drawFireworkTimer,
    drawFireworkCounter = 0, fireworkRowNumber = 8,
    fireworkColumnNumber = 16, fireworkLayerNumber = 0, fireworkDotRadius = 5,
    fireworkCenterX, fireworkCenterY,
    fireworkOneRadiusDistance, fireworkOneRotationAngle,
    drawOneLayerOfFireworkTimer, canDrawManyFireworks = !0;
disableIsRobbyJumpingAndFalling();
